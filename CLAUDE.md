...
- Pages: `/` (KPIs + Leaflet map + largest-facilities), `/analyse` (Versorgung/Qualität/Bike+Ride/Datenqualität tabs, plus a peer-group chip filter that applies across all four), `/vergleich` (OSM vs. Stadt Karlsruhe), `/progress` (history chart), `/about`.
